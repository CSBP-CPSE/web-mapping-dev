(function () {
	...
	var nls = {
	  ...
	  "Toc_Legend": {
	    ...
	    "fr": "Proportion population à bâtiment"
	  },
	  "Toc_Opacity": {
	    "en": "Opacity",
	    "fr": "Opacité"
	  ...
	  "popup_area_vert": {
	    ...
	    "fr": "Rapport de la surface aux sommets" //Not sure about these translations...

	  },
	  "popup_peri_vert": {
	    "en": "Perimeter to vertices ratio",
	    "fr": "Rapport du périmètre aux sommets"
	  }
	};
	...
	function () {
	  ...
	  _createClass(Utils, null, [{
	    key: "HTMLize",
	    value: function HTMLize(json, prefix) {
	      var html = "";

	      for (var key in json) {
	        if (key == "tilequery") continue;
	        var label = Core.Nls(prefix + key);
	        html += "<div class='row'><span>".concat(label, " : </span><span>").concat(json[key], "</span></div>");
	      }
	...
	function (_Control) {
	  ...
	  function Legend(options) {
	    ...
	    _this.radios = {}; //https://www.dyn-web.com/tutorials/object-literal/properties.php

	    for (var _i = 0, _radioLayers = radioLayers; _i < _radioLayers.length; _i++) {
	      var k = _radioLayers[_i];
	      _this.radios[k] = _this.Node('rd' + k.toUpperCase());
	    }
	    _this.current = 'csd';
	    _this.radios[_this.current].checked = true; //https://www.w3schools.com/jsref/jsref_forin.asp

	    for (var t in _this.radios) {
	      _this.radios[t].addEventListener('change', _this.onChange_Handler.bind(_assertThisInitialized(_this), t));
	    }
	  ...
	  _createClass(Legend, [{
	    ...
	    value: function Template() {
	      ...
	      for (var _i2 = 0, _radioLayers2 = radioLayers; _i2 < _radioLayers2.length; _i2++) {
	        var w = _radioLayers2[_i2];
	        radioHTML += "<label><input handle='rd" + w.toUpperCase() + "' type='radio' name='boundary'>nls(Label_" + w.toUpperCase() + ")</label>";
	      }
	      return "<div handle='root' class='legend mapboxgl-ctrl mapboxgl-ctrl-group'>" + "<div class='legend-label'>nls(Toc_Legend)</div>" + "<div class='legend-container'>" + "<div class='legend-item legend-item-1'>" + "<div class='legend-icon'></div>" + "<div>nls(Legend_Item_1)</div>" + "</div>" + "<div class='legend-item legend-item-2'>" + "<div class='legend-icon'></div>" + "<div>nls(Legend_Item_2)</div>" + "</div>" + "</div>" + "<div class='legend-container'>" + "<div class='legend-item legend-item-3'>" + "<div class='legend-icon'></div>" + "<div>nls(Legend_Item_3)</div>" + "</div>" + "<div class='legend-item legend-item-4'>" + "<div class='legend-icon'></div>" + "<div>nls(Legend_Item_4)</div>" + "</div>" + "</div>" + "<div class='legend-container'>" + "<div class='legend-item legend-item-5'>" + "<div class='legend-icon'></div>" + "<div>nls(Legend_Item_5)</div>" + "</div>" + "<div class='legend-item legend-item-6'>" + "<div class='legend-icon'></div>" + "<div>nls(Legend_Item_6)</div>" + "</div>" + "<div class='legend-item legend-item-7'>" + "<div class='legend-icon'></div>" + "<div>nls(Legend_Item_7)</div>" + "</div>" + "</div>" + "<div class='legend-label'>nls(Toc_Instruction)</div>" + "<div class='legend-container toc-container'>" + radioHTML + "</div>" + "<div class='legend-label'>nls(Toc_Opacity)</div>" + "<div class='opacity-slider'>" + "<input handle='slider' type='range' min='0' max='100' value='50' class='slider'>" + "</div>" + "</div>";
	    }
	  }]);
	...
	function Initialize() {
	  ...
	  var legend = Factory.LegendControl();
	  ...
	  map.on('load', function (ev) {
	    ...
	    var classes = Classes(legend.opacity);
	    ...
	    config.layers.forEach(function (l) {
	      map.on('click', l, function (ev) {
	        ...
	        var html = Utils.HTMLize(ev.features[0].properties, 'popup_');
	  ...
	  function OnLegend_OpacityChanged(ev) {
	    var classes = Classes(ev.opacity);
	    Map.Choropleth(map, config.layers, classes);
	  }

	  function Classes(opacity) {
	    return ['case', ['==', ['/', ['get', 'population'], ['get', 'buildings']], 0], "rgba(246,239,247,".concat(opacity, ")"), ['<', ['/', ['get', 'population'], ['get', 'buildings']], 1], "rgba(208,209,230,".concat(opacity, ")"), ['<', ['/', ['get', 'population'], ['get', 'buildings']], 2], "rgba(166,189,219,".concat(opacity, ")"), ['<', ['/', ['get', 'population'], ['get', 'buildings']], 5], "rgba(103,169,207,".concat(opacity, ")"), ['<', ['/', ['get', 'population'], ['get', 'buildings']], 10], "rgba(28,144,153,".concat(opacity, ")"), ['<', ['/', ['get', 'population'], ['get', 'buildings']], 50], "rgba(1,108,89,".concat(opacity, ")"), 'rgba(0, 0, 0, 0)'];
	  }
	}
