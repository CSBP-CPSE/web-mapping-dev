(function () {
    ...
    var config$4 = {
      layers: ["db"],
      title: {
        "en": "Proximity to public transit trips<br>(preliminary results, subject to revision)",
        "fr": "Proximité aux voyages de transport en commun<br>(résultats préliminaires, sujet à révision)"
      },
      legend: [{
        color: [204, 255, 51],
        label: {
    ...
    function (_Control) {
      ...
      function Legend(options) {
        ...
        _this2 = _possibleConstructorReturn(this, _getPrototypeOf(Legend).call(this, options));
        _this2._container = _this2.Node('root');
        _this2.radios = {};

        if (_this2.options.toc) {
          _this2.AssignRadioButtons(_this2.options.toc);

          if (_this2.options.selected) {
            _this2.current = _this.options.selected;
            _this2.radios[_this2.options.selected].checked = true;
          }
        }

        if (_this2.options.title && _this2.Node('title')) {
          _this2.Node('title').innerHTML = _this2.options.title[Core.locale];
        }

        _this2.Node('slider').addEventListener("change", _this2.onSliderChange_Handler.bind(_assertThisInitialized(_this2)));
